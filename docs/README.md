...
如果希望获得更稳定、更有表现力的视频效果，可以选择使用 **H3 Prompt Writing Skill**，让 Codex 或其他 agent 根据你的创意生成结构化的视频提示词。

该 skill 支持文生视频（T2VA）、图生视频（I2VA）、首尾帧生视频（FL2VA/L2VA）和全参考生视频（Ref2VA），会整理镜头构图、主体动作、环境、摄影机运动、声音、音乐及时间线等信息。使用时可以直接告诉 Codex：

> 使用 H3 Prompt Writing Skill，把我的创意改写成适用于 [视频模式] 的结构化提示词，视频时长为 [时长]，并保留 [对白/歌词/画面文字] 的原文。

生成提示词后，将对应的结构复制到视频生成页面的提示词输入框中即可。
